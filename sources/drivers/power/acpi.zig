                ...
                if (checkHeader(facp.DSDT, "DSDT")) {
                    ...
                    if (dsdt_length > 0) {
                        if ((@as(*u8, @ptrFromInt(@intFromPtr(S5_addr) - 1)).* == 0x08 or
                            (@as(*u8, @ptrFromInt(@intFromPtr(S5_addr) - 2)).* == 0x08 and @as(*u8, @ptrFromInt(@intFromPtr(S5_addr) - 1)).* == '\\')) and @as(*u8, @ptrFromInt(@intFromPtr(S5_addr) + 4)).* == 0x12)
                        {
                            S5_addr += 5;
                            ...
                            ACPI_init = true;
                            return true;
                        } else kernel.logs.klogln("[ACPI] \\_S5 parse error");
                    } else kernel.logs.klogln("[ACPI] \\_S5 not present");
                } else kernel.logs.klogln("[ACPI] DSDT invalid");
            }
            ptr = @ptrFromInt(@intFromPtr(ptr) + @sizeOf(u32));
        }
        kernel.logs.klogln("[ACPI] no valid FACP present");
    } else kernel.logs.klogln("[ACPI] No RSDT found");
    return false;
}
